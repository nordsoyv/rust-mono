...
pub fn three<'a, P1, P2, P3, R1, R2, R3>(parser1: P1, parser2: P2, parser3: P3) -> impl Parser<'a, (R1, R2, R3)>
    where
        P1: Parser<'a, R1>,
        P2: Parser<'a, R2>,
        P3: Parser<'a, R3>,
{
    move |input| {
        parser1.parse(input).and_then(|(second_input, result1)| {
            parser2.parse(second_input).and_then(|(third_input, result2)| {
                parser3.parse(third_input).map(|(last_input, result3)| (last_input, (result1, result2, result3)))
            })
        })
    }
}

pub fn four<'a, P1, P2, P3, P4, R1, R2, R3, R4>(parser1: P1, parser2: P2, parser3: P3, parser4: P4) -> impl Parser<'a, (R1, R2, R3, R4)>
    where
        P1: Parser<'a, R1>,
        P2: Parser<'a, R2>,
        P3: Parser<'a, R3>,
        P4: Parser<'a, R4>,
{
    move |input| {
        parser1.parse(input).and_then(|(second_input, result1)| {
            parser2.parse(second_input).and_then(|(third_input, result2)| {
                parser3.parse(third_input).and_then(|(fourth_input, result3)| {
                    parser4.parse(fourth_input).map(|(last_input, result4)| (last_input, (result1, result2, result3, result4)))
                })
            })
        })
    }

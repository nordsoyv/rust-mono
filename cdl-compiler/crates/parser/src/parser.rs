...
use crate::{
  ast_nodes::{
    ast_boolean::AstBooleanNode, ast_function::AstFunctionNode, AstColorNode, AstEntityNode, AstFormulaNode, AstIdentifierNode, AstNumberNode, AstOperatorNode, AstPropertyNode, AstReferenceNode, AstScriptNode, AstStringNode, AstTableAliasNode, AstTitleNode, AstVPathNode, Parsable
  },
...
impl Parser {
  pub fn start_group(& self, text: String) {
    ...
    let indent = (0..self.log_groups.borrow().len()).map(|_| " |").collect::<String>();
    trace!(target: "parsing event", "{} {}", indent, text );
    self.log_groups.borrow_mut().push(LogGroup{name: text});
    
  }
  pub fn trace(&self, text: &str) {
    let i = (0..self.log_groups.borrow().len()).map(|_| " |").collect::<String>();

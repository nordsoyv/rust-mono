...
pub fn draw_input(entity: &Entity, em: &EntityManager) {
  let parent = em.get_entity(entity.get_parent_id().unwrap());
  if let Some(in_data) = entity.input_data.as_ref()
    && let Some(ui_data) = parent.borrow().ui_data.as_ref()
  {
    let mouse_pos = Vec2::new(mouse_position().0, mouse_position().1);
    let center = Vec2::new(
      in_data.offset_x + ui_data.x_pos,
      in_data.offset_y + ui_data.y_pos,
    );
